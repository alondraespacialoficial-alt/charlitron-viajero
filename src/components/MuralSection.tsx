    ...
    <div className="min-h-screen bg-sepia-950 pt-24">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 pb-24">
        {isLoading ? (
          ...
          /* Masonry-style responsive collage */
          <div className="columns-2 sm:columns-3 lg:columns-4 gap-3 md:gap-4 space-y-0">
            {photos.map((photo, index) => (
              <motion.div
                key={photo.id}
                ...
                onClick={() => setActiveIndex(index)}
              >
                <div className={`relative overflow-hidden rounded-2xl border border-sepia-800/60 shadow-lg transition-all duration-500 group-hover:border-sepia-600/60 group-hover:shadow-sepia-900/40 group-hover:shadow-2xl group-hover:scale-[1.02] ${photo.is_vertical ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
                  <img
                    src={photo.photo_url}
                    alt={photo.person_name}
                    className="w-full h-full object-cover sepia-[0.2] group-hover:sepia-0 transition-all duration-700 select-none pointer-events-none"
                    referrerPolicy="no-referrer"
                    onContextMenu={(e) => e.preventDefault()}
                    onDragStart={(e) => e.preventDefault()}
                  />
                  ...
                  <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-5 group-hover:opacity-10 transition-opacity">
                    <span className="text-white text-xs font-serif tracking-widest rotate-[-20deg]">Charlitron®</span>
                  </div>

                  {/* Top-corner badge */}
                  <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                    <div className="bg-sepia-950/70 backdrop-blur-sm rounded-full p-1.5">
                      <Camera className="w-3 h-3 text-sepia-300" />
                    </div>
                  </div>

                  {/* Gradient overlay with name */}
                  <div className="absolute inset-0 bg-gradient-to-t from-sepia-950/90 via-sepia-950/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-400 flex flex-col justify-end p-4">
                    <p className="text-sepia-100 font-serif text-base leading-tight">{photo.person_name}</p>
                    {photo.encounter_text && (
                      <p className="text-sepia-300 text-xs italic mt-1 line-clamp-2">{photo.encounter_text}</p>
                    )}
                  </div>
                </div>
              </motion.div>
        ...
        {activeIndex !== null && (
          ...
          >
            ...
            <motion.div
              ...
              transition={{ duration: 0.3 }}
              className="relative max-w-2xl w-full max-h-[88vh] flex flex-col items-center gap-5"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Image */}
              <div className={`relative overflow-hidden rounded-2xl border-2 border-sepia-700/60 shadow-2xl w-full ${photos[activeIndex].is_vertical ? 'max-h-[70vh]' : ''}`}>
                <img
                  src={photos[activeIndex].photo_url}
                  alt={photos[activeIndex].person_name}
                ...
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-10">
                  <div className="rotate-[-25deg] border-2 border-white/30 p-6 flex flex-col items-center">
                    <span className="text-white text-3xl font-serif tracking-widest">Charlitron®</span>
                  </div>
                </div>
              </div>
              ...
              <motion.div
                ...
                className="w-full bg-sepia-900/80 backdrop-blur-sm rounded-2xl border border-sepia-800/60 px-6 py-5 text-center"
              >
                <p className="text-sepia-100 font-serif text-xl mb-1">{photos[activeIndex].person_name}</p>
                {photos[activeIndex].encounter_text && (
                  <p className="text-sepia-400 text-sm italic font-light leading-relaxed">{photos[activeIndex].encounter_text}</p>
                )}
              </motion.div>
            </motion.div>
